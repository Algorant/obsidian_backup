...
```bash
 axicli --mode manual --manual_cmd walk_home --config .\axidraw_conf.py
```

#### Toggle up and down

```bash
axicli --mode cycle --config .\axidraw_conf.py
```
...
```bash
axicli -m manual -M "write_nameAxiDraw"
```

#### Enable / Disable Motors

```bash
axicli -m manual -M enable_xy
axicli -m manual -M disable_xy
```

#### Pen heights for IDraw
- current: 25/50
---


### Getting it working in WSL

- Make sure /dev/ttyACM0 is visible
- Add read/write permissions, I used `sudo chmod a+rw /dev/ttyACM0`

- Install usbipd using winget on the windows side
- run `usbipd list` and look for a thing called `4-2    04d8:fd92  USB Serial (UBW-based) communications port (COM3)             Not shared`. This was the [[AxiDraw]] on my machine.
- You need powershell with admin to bind it to [[WSL]] with these commands
	- `usbipd bind --busid 4-2` it was 4-2 in my thing, might be diff
	- `usbipd attach --wsl --busid=4-2` again, might be different
...
pen_pos_up = 50         # Height of pen when raised (0-100). Default 60
pen_pos_down = 20       # Height of pen when lowered (0-100). Default 30
...
progress = True        # Enable progress bar display in AxiDraw CLI, when True
                            # Default False
